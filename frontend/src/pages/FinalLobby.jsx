...
export default function FinalLobby() {
  ...
  const handleSetReady = async () => {
    try {
      ...
      alert('✅ Ваш отдел готов к финалу!');
      await loadLobby();
    } catch (err) {
  ...
  const allReady = teams.length === 3 && teams.every(t => t.isReady);
      ...
      <div className="rating-card fade-in" style={{ maxWidth: '900px' }}>
        ...
        {myTeam && myTeamReady && (
          <div className="text-center" style={{ padding: '16px', background: 'rgba(16,185,129,0.1)', borderRadius: '16px' }}>
            <span style={{ color: '#10b981', fontSize: '16px', fontWeight: 'bold' }}>✓ Ваш отдел готов к финалу</span>
          </div>
        )}
